...
test('a local dispatch resolves its model visibly, is cancellable while resolving, never queues, and binds reports', {timeout: 3000}, async t => {
  ...
  assert.equal(launches, 2, 'two local workers run at once: there is no capacity queue');
  ...
  // Cancelled while still resolving: nothing is launched.
