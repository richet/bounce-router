...
// warning), wait with a warning when nothing fits, and never start local work under memory pressure —
...
const machine = over => ({known: true, ramTotal: 128 * GB, available: 30 * GB, wiredLimit: 92 * GB, pressure: 1, swapUsed: 8 * GB, at: 0, ...over});

test('G1 a machine under memory pressure does not run a local worker: it warns and the next AI takes the task', async t => {
  ...
  const scheduler = createScheduler({session, adapters: {opencode, claude}, localResolver, gitHead: () => null,
    ...
    resources: {read: () => machine({pressure: 2}), underPressure: () => true},
  ...
  assert.equal(failed.text, 'the machine is under warning memory pressure: a local worker would make it worse');
...
test('G2 an idle resident model is unloaded to make room, with a warning naming what it freed', async t => {
  ...
  const scheduler = createScheduler({session, adapters: {opencode}, localResolver, gitHead: () => null,
    ...
    resources: {read: () => machine({available: 20 * GB}), underPressure: () => false},
...
test('G3 when nothing fits the task waits with a warning, is escalated if it waits too long, and runs when memory frees', async t => {
  ...
  const scheduler = createScheduler({session, adapters: {opencode}, localResolver, gitHead: () => null, clock: () => now,
    profiles: {b: localProfile('big')},
    resources: {read: () => machine({available}), underPressure: () => false},
  ...
  assert.equal(session.events.some(e => e.kind === 'policy.escalated' && e.task === row.task && e.reason === 'startup_timeout'), false);
  available = 60 * GB;
  await waitFor(() => scheduler.tasks()[row.task]?.state === 'completed');
});

test('G4 provider startup allowance begins after intentional memory admission completes', async t => {
  const session = setup(t);
  let available = 10 * GB, now = 0;
  const entered = Promise.withResolvers();
  const opencode = fakeAdapter(({signal}) => new Promise((resolve, reject) => {
    entered.resolve();
    signal.addEventListener('abort', () => reject(new Error('launch aborted')), {once: true});
  }));
  const scheduler = createScheduler({session, adapters: {opencode}, localResolver, gitHead: () => null, clock: () => now,
    watchdog: {interval: null, startupMs: 2 * 60000}, profiles: {b: localProfile('big')},
    resources: {read: () => machine({available}), underPressure: () => false},
    localFleet: async () => catalogs([model('big', {size: 45 * GB})]),
    localSettings: {endpoints: {lmstudio: {backend: 'lmstudio', url: 'http://127.0.0.1:1234', maxConcurrent: 2, waitMinutes: 10, pollMs: 5}}}});
  t.after(() => scheduler.close());
  const row = scheduler.submit({parent: null, profile: 'b', orders: 'build it', deadline: null});
  await waitFor(() => session.events.some(e => e.kind === 'task.milestone' && e.task === row.task && /memory/.test(e.text)));
  now = 11 * 60000;
  available = 60 * GB;
  ...
  now++;
  await scheduler.tick();
  assert.equal(session.events.some(e => e.kind === 'policy.escalated' && e.task === row.task && e.reason === 'startup_timeout'), true);
});

// Observed live (2026-09-25): a 128 GB machine at macOS memory pressure level 1 (normal, 85% free) was
// called "swapping" because vm_stat's Swapouts counter rose by a few pages between two admissions
// minutes apart; the report repair and a new task were refused. Background swap-outs are not pressure.
test('G5 background swap-outs at normal memory pressure do not stop local work', async t => {
  const session = setup(t);
  let swapouts = 23205262;
  const run = (cmd, args) => {
    if (cmd === 'vm_stat') return `Mach Virtual Memory Statistics: (page size of 16384 bytes)\nPages free: 191336.\nPages inactive: 2983304.\nPages wired down: 405635.\nPages purgeable: 37647.\nSwapouts: ${swapouts += 40}.\n`;
    if (args?.[1] === 'hw.memsize') return String(128 * GB);
    if (args?.[1] === 'vm.swapusage') return 'total = 5120.00M  used = 3973.75M  free = 1146.25M  (encrypted)\n';
    if (args?.[1] === 'iogpu.wired_limit_mb') return '92000';
    if (args?.[1] === 'kern.memorystatus_vm_pressure_level') return '1\n';
    throw new Error(`unexpected ${cmd} ${args?.join(' ')}`);
  };
  const opencode = fakeAdapter(() => [{kind: 'result', status: 'completed', text: 'done'}]);
  const scheduler = createScheduler({session, adapters: {opencode}, localResolver, gitHead: () => null, profiles: {b: localProfile('m')},
    resources: createResources({run, ttlMs: 0}), localFleet: async () => catalogs([model('m', {size: 4 * GB, ready: true})]), unloadLocal: async () => {}});
  t.after(() => scheduler.close());
  for (const orders of ['first', 'second', 'third']) {
    const row = scheduler.submit({parent: null, profile: 'b', orders, deadline: null});
    await waitFor(() => ['completed', 'failed'].includes(scheduler.tasks()[row.task]?.state));
    assert.equal(scheduler.tasks()[row.task].state, 'completed', session.events.findLast(e => e.task === row.task && e.kind === 'task.failed')?.text);
  }
});
