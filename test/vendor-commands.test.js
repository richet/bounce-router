...
test('the TUI treats an agent-owned /NAME as a turn, offers it in the picker and runs it on Enter', () => {
  ...
  assert.deepEqual(completions('/s', extra).map(r => r[0]), ['steer', 'sessions', 'skills', 'sidebar', 'stop', 'seed-account']);
  assert.equal(completions('/', extra).length, 33, 'own commands, then the two extras that do not clash');
