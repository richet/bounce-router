...
// one-shot fake, with isolated writable attempts and verified artifact publication.
...
test('a local write worker publishes its isolated edit, and its final envelope completes the task', async t => {
  ...
  assert.equal(session.events.find(e => e.kind === 'task.completed' && e.task === row.task).summary, '{"op":"final","phase":"complete","text":"completed set x to 2","next":"","evidence":[],"outcome":"completed","summary":"completed set x to 2","remaining":""}');
});

test('under a required final report, a local worker completes only with its valid final envelope', async t => {
  ...
  assert.equal(done.summary, 'completed scout');
  assert.equal(done.synthesized, undefined);
...
test('a cloud worker that hits its limit falls back to a local worker, task preserved', async t => {
  ...
  await waitFor(() => scheduler.tasks()[retry.task].state === 'completed');
  assert.match(session.events.find(e => e.kind === 'task.completed' && e.task === retry.task).summary, /^\{"op":"final"/);
});
...
test('a granted local worker is told to report through its bounce_report tool, not the CLI', async t => {
  const {session} = setup(t);
  // A write-policy worker now runs fenced off the real checkout (src/scheduler.js writeFence), so a log
  // path must live outside `root` — inside it, the fake helper's own write would be refused by the fence.
  const logDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'oc-integration-log-')));
  const log = path.join(logDir, 'fake.log'); process.env.FAKE_OC_LOG = log;
  t.after(() => { delete process.env.FAKE_OC_LOG; fs.rmSync(logDir, {recursive: true, force: true}); });
  ...
  assert.equal(prompt.includes('call the bounce_report tool with the report object'), true, prompt);
  assert.equal(prompt.includes('bounce report --report'), false, prompt);
});
