...
export function choosingOrders({agents = false, routingOn = false, localOn = false} = {}) {
  ...
  return ['Who to submit to — the job, not the AI:',
    ...
    'A live container check (starting Docker, inspecting or running one) needs requires including "docker" as well, and only a profile that can actually honor it accepts the task — a read-only analyst without exec cannot reach Docker either way, and a probe analyst on an adapter whose sandbox cannot allow-list the socket is refused at submit with a capability mismatch.',
...
async function daemonSupervise(args, {spawnChild, updateInstall, adapters: extraAdapters = {}, profiles: profileOverride, strategy: strategyOverride, onReady} = {}) {
  ...
  bus = await createBus({session, dir: session.dir, validate: scheduler.validate, prepare: scheduler.prepare, report: scheduler.report, accept: scheduler.acceptOverride, rework: scheduler.reworkOverride, reworkAvailable: scheduler.roundsAvailable});
