  ...
  async function callTool(name, args = {}) {
    ...
    if (name === 'task_submit') {
      ...
      const result = await ops.submit({kind: 'task.submitted', task: args.task, profile: args.profile, orders: args.orders, requires: args.requires, parent: args.parent ?? null,
        jobId: args.jobId, retryOf: args.retryOf, campaignId: args.campaignId, gate: args.gate, planId: args.planId, chunkId: args.chunkId, ...Object.fromEntries(['owns', 'depends_on', 'deadline', 'review', 'steps', 'risk', 'size', 'checkpoint'].filter(key => args[key] !== undefined).map(key => [key, args[key]]))});
      return result.ok ? ok(rowText(result.row), result.row) : fail(`bounce refused it: ${result.reason}`, result);
    }
