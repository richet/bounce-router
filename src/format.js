...
const WORKER_ROW = {
  ...
  'task.finding': e => ({kind: 'note', text: `finding · ${e.finding ? [e.finding.severity, e.finding.file && `${e.finding.file}${e.finding.line ? `:${e.finding.line}` : ''}`, e.finding.title].filter(Boolean).join(' ') : e.text.replace(/^FINDING:\s*/, '')}`}),
  'task.lease.renewed': e => ({kind: 'status', text: `lease ${e.lease} renewed${e.jev?.verdict ? ` · Jev: ${e.jev.verdict}` : ''}`}),
  'task.concluding': e => ({kind: 'status', text: `asked for its conclusion · ${e.reason.replace('_', ' ')}`}),
