...
export function previewLocalProfile({settings, name, options = {}}) {
  ...
  // Taken names are read off the validated table, so a shipped profile the config never
  // names (build) is refused too rather than silently overridden by a local worker.
  if (Object.hasOwn(validateOrchestration(settings).profiles, name)) throw new Error(`Profile ${name} already exists; edit its config or use /model worker ${name}`);
