...
When the orchestrator's answer ends by proposing what to do next (`Next: …`, `Should I …?`), that step is prefilled dim in the prompt: Tab takes it, typing replaces it, and it is never sent on its own. An open question, or a list of steps, is not prefilled. The proposal is read from the turn's last answer, on a turn you started and on one the daemon woke alike, and only when the turn completed: a cancelled turn prefills nothing.
...
- Ctrl+O (or `/operation` with no argument) opens the classic/orchestrator menu; choosing the other mode restarts the session into it. See [Orchestrator mode](#orchestrator-mode).
...
- `/model MODEL_ID` saves the selected provider's model without opening the picker; `/model default` uses its native default. In orchestrator mode a pick, `/model MODEL_ID` and `/order` all change the orchestrator itself: the profile named by `orchestrator` takes that adapter and model and the next turn runs on it (the classic order and per-agent models are what classic mode runs on, and used to be all these commands wrote).
...
- `/order claude,codex,muse` saves routing order. Omit a provider to disable it. Bare `/order` prints the order in effect with each provider's model. In orchestrator mode the first agent becomes the orchestrator, with the model already chosen for that agent, and the rest is its fallback.
...
`/operation orchestrator` (or Ctrl+O, then Enter) switches a session from *classic* — one agent answers each turn, with fallback — to *orchestrator*: the main agent coordinates and never edits the repository itself. It reads a standing brief bounce writes at `sessions/<id>/orchestrator/ORDERS.md`, submits tasks to worker profiles over the session bus, and reports outcomes to you. Claude's own Agent/Task tools are switched off for it, and ORDERS.md forbids the other vendors' subagent features, so all delegation is visible in bounce. Switching back is `/operation classic`. The mode is fixed per process (the daemon and its workers exist or they don't), so a switch saves the choice to `config.json` and restarts the session into the new mode — same session id and transcript — and is refused while workers are still running (`/stop` first). The running mode is shown in the sidebar and `bounce sessions`.
...
bounce wait --match '{"kind":"task.completed","task":"TASK_ID"}' --timeout 120
...
However large the request, the orchestrator does not hand one worker the whole job. Its standing orders tell it to plan **phases in sequence, each made of chunks that run in parallel** where their owned paths are disjoint, to chain phases with `depends_on`, and to review each phase before the next starts. Chunks are sized by scope (one owner, one acceptance), not by minutes. A task's deadline is a **lease**: `taskMinutes` (config.json, default 15, 1–240) is its length, and a task with no deadline gets it. When a lease runs out, bounce renews it if the worker made progress in it and is not repeating earlier tool calls (and, with Jev on, Jev does not judge it stuck; `drifting` renews and tells the submitter). Otherwise, or at `taskCeilingMinutes` (default 60, at least `taskMinutes`, measured from the first start), the worker is asked for its conclusion: OpenCode stops the turn and asks the same session with its tools off, and other workers get a message. A worker that shows no activity for five minutes while asked, or gives no answer within fifteen, is cancelled; OpenCode heartbeats a step that is still generating. A reviewer writes each finding as a `FINDING:` line when it confirms it, and bounce keeps them as `task.finding` rows, so a stopped review hands its findings on in its deadline row. A `probe` agent (the shipped reviewer) runs commands but cannot change the project: on codex through its read-only sandbox, locally under macOS `sandbox-exec` (no project writes, localhost-only network, no Docker socket); claude and muse cannot run one and are skipped for it. A deadline over the ceiling is refused before anything runs (`task.failed`, reason `size`). Found live: a hard 15-minute cap killed every local 27B reviewer mid-review, each one 11–84 s after its last step.

**A plan is judged before its chunks run.** The orchestrator submits a phase's breakdown as `plan.submitted` (chunks with `id`, `profile`, `orders`, `owns`, `depends_on`, `deadline`). bounce refuses overlapping owned paths and a deadline over the ceiling itself; with Jev on, each chunk is also judged for being phase-sized, having no acceptance criterion, or a hidden dependency on another chunk. The answer is `plan.accepted` or `plan.rejected` with findings per chunk and the fix. Live, real Jev: a clean three-chunk plan accepted with no findings in 308 ms; a plan with a "finish P2" chunk, an audit with no acceptance and two chunks on the same paths rejected for exactly those. The gate warns; it does not yet refuse chunks submitted without an accepted plan.
...
Agent files are layered, later ones shadowing by name: the four shipped with bounce (`analyst`, `builder`, `debugger`, `reviewer`) → `~/.bounce/agents/` → `<workspace>/.bounce/agents/`.
...
`local.endpoints.<name>.contextTokens` (optional, at least 4096) is the context OpenCode is told each local model has: a worker's conversation is compacted at that size instead of growing to whatever LM Studio loaded the model with (these MLX builds load at 262k whatever the CLI asks). Verified live: with a 32k limit a worker reading a 60k-token file compacted twice and still answered correctly; without it the prompt grew to 82k. `local.endpoints.<name>.maxConcurrent` (default 1) is how many local workers run on that endpoint at once; it is the ceiling for the endpoint as a whole. `slotsPerModel` (optional) is the limit per model within it, set to the parallel slots LM Studio gives each loaded model: two builders on one model no longer hold back a reviewer on the other, idle one (observed: a reviewer queued seven minutes behind two builders on a different model). A task past either limit stays queued, says which limit and how many are in use, and starts when a local turn ends. Cloud workers are never held by either — they share one machine-wide `maxConcurrentCloud` (config.json, default 3) instead, since a cloud vendor and a local endpoint have nothing in common to size against.
...
**A deadline is bounce's decision, not the user's.** A task that gives no conclusion after its lease ends is cancelled with reason `deadline`, and whoever submitted it is told, with the way forward: its partial progress is in the journal, resubmit what is left with that progress in the orders, or drop it. The orders say the same; only a cancellation with reason `user` means stop.
...
- `config.json`: order, mode, per-provider models, cooldownMinutes, contextChars, `maxConcurrentCloud`, executable overrides, skill scope and auto-sync, `jev` (never its key); in orchestrator mode also `operation`, `orchestrator`, `profiles`, `strategy`, `strict` and `local`.
