...
Live progress — Claude's thinking-token counters, tool heartbeats and the start and end of each foreground Bash call, Codex's command starts — is shown on the status line beside the spinner and is never written to the transcript or the journal, so a long turn no longer buries the conversation in repeated `thinking_tokens` blocks. Short bookkeeping events (agent selected, activity, cooldown, agent finished, turn finished) render as a single line. `/details` shows everything: every tool call field by field with real newlines rather than as escaped JSON, full tool output, and worker dispatch (`/details on|off` sets it explicitly); the journal keeps the original text for handoffs.
